	...
	if(Parser.parse_file(FileName,pug::parse_trim_attribute))
	{
		...
		if(Config.moveto_child("Main"))
		{
			...
			if(Object.moveto_child("Object"))
			{
				...
				if (Object.has_attribute("ObjectGroupProcessLimit"))
					m_DOSConfig.ObjectConfig.ObjectGroupProcessLimit = Object.attribute("ObjectGroupProcessLimit");
				...
				//m_DOSConfig.ObjectConfig.pDOSGroupInitFN = CCSVMHelper::DosGroupInitFn;
				//m_DOSConfig.ObjectConfig.pDOSGroupDestoryFN = CCSVMHelper::DosGroupDestoryFn;
			}
			...
			if (Mono.moveto_child("Mono"))
			{
				...
				{
					m_MonoConfig.Enable = Mono.attribute("Enable");
				}
#ifndef WIN32
				if (Mono.has_attribute("MonoModuleFileNameForLinux"))
				{
					m_MonoConfig.MonoModuleFileName = Mono.attribute("MonoModuleFileNameForLinux").getvalue();
				}
				else
#endif
				if (Mono.has_attribute("MonoModuleFileName"))
				{
					m_MonoConfig.MonoModuleFileName = Mono.attribute("MonoModuleFileName").getvalue();
				}
				if (Mono.has_attribute("DOSSystemAssemblyFileName"))
				{
					m_MonoConfig.DOSSystemAssemblyFileName = Mono.attribute("DOSSystemAssemblyFileName").getvalue();
				}
#ifndef WIN32
				if (Mono.has_attribute("LibraryDirForLinux"))
				{
					m_MonoConfig.LibraryDir = Mono.attribute("LibraryDirForLinux").getvalue();
				}
				else
#else
...
#endif
				if (Mono.has_attribute("ConfigDir"))
				{
					m_MonoConfig.ConfigDir = Mono.attribute("ConfigDir").getvalue();
				}
				...
				if (Mono.has_attribute("Options"))
				{
					m_MonoConfig.Options = Mono.attribute("Options").getvalue();
				}
				if (Mono.has_attribute("EnableDebug"))
				{
					m_MonoConfig.EnableDebug = Mono.attribute("EnableDebug");
				}
				if (Mono.has_attribute("DebugListenHost"))
				{
					m_MonoConfig.DebugListenHost = Mono.attribute("DebugListenHost").getvalue();
				}
				if (Mono.has_attribute("DebugListenPort"))
				{
					m_MonoConfig.DebugListenPort = Mono.attribute("DebugListenPort");
				}
				...
				xml_node PluginObjectPool = Mono;
				if (PluginObjectPool.moveto_child("PluginObjectPool"))
				{
					ReadPoolConfig(PluginObjectPool, m_MonoConfig.PluginObjectPoolConfig);
				}
			}
			xml_node JSVM = Config;
			if (JSVM.moveto_child("JSVM"))
			{
				if (JSVM.has_attribute("Enable"))
				{
					m_JSVMConfig.Enable = JSVM.attribute("Enable");
				}
#ifndef WIN32
				if (JSVM.has_attribute("JSVMModuleFileNameForLinux"))
				{
					m_JSVMConfig.JSVMModuleFileName = JSVM.attribute("JSVMModuleFileNameForLinux").getvalue();
				}
				else
#endif
				if (JSVM.has_attribute("JSVMModuleFileName"))
				{
					m_JSVMConfig.JSVMModuleFileName = JSVM.attribute("JSVMModuleFileName").getvalue();
				}
				if (JSVM.has_attribute("Options"))
				{
					m_JSVMConfig.Options = JSVM.attribute("Options").getvalue();
				}
				if (JSVM.has_attribute("EnableDebug"))
				{
					m_JSVMConfig.EnableDebug = JSVM.attribute("EnableDebug");
				}
				if (JSVM.has_attribute("IsDebugSuspend"))
				{
					m_JSVMConfig.IsDebugSuspend = JSVM.attribute("IsDebugSuspend");
				}
				if (JSVM.has_attribute("DebugListenHost"))
				{
					m_JSVMConfig.DebugListenHost = JSVM.attribute("DebugListenHost").getvalue();
				}
				if (JSVM.has_attribute("DebugListenPort"))
				{
					m_JSVMConfig.DebugListenPort = JSVM.attribute("DebugListenPort");
				}
				if (JSVM.has_attribute("InitiativeGCInterval"))
				{
					m_JSVMConfig.InitiativeGCInterval = JSVM.attribute("InitiativeGCInterval");
				}
				if (JSVM.has_attribute("GCLevel"))
				{
					m_JSVMConfig.GCLevel = JSVM.attribute("GCLevel");
				}				
				if (JSVM.has_attribute("FullGC"))
				{
					m_JSVMConfig.FullGC = JSVM.attribute("FullGC");
				}

				xml_node PluginObjectPool = JSVM;
				if (PluginObjectPool.moveto_child("PluginObjectPool"))
				{
					ReadPoolConfig(PluginObjectPool, m_JSVMConfig.PluginObjectPoolConfig);
				}
			}
	...
	for (UINT i = 0; i < XMLContent.children(); i++)
	{
		...
		else if (_stricmp(Plugin.name(), "Plugin") == 0)
		{
			...
			if (Plugin.has_attribute("Type"))
			{
				...
				if (TypeName.CompareNoCase("CS") == 0)
				{
					PluginInfo.PluginType = PLUGIN_TYPE_CS;
				}					
				else if (TypeName.CompareNoCase("JS") == 0)
				{
					PluginInfo.PluginType = PLUGIN_TYPE_JS;
				}
				else
				{
					PluginInfo.PluginType = PLUGIN_TYPE_NATIVE;
				}
			}				
