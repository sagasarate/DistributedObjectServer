...
namespace DOSSample
{
	public class CShipyardMsgCaller :
		IShipyard
	{
		...
		public CShipyardMsgCaller(DistributedObjectOperator Operator, OBJECT_ID SenderID, OBJECT_ID TargetID)
		{
			m_Operator = Operator;
			m_SenderID = SenderID;
			m_TargetIDList = new OBJECT_ID[1]{TargetID};
		}
		public CShipyardMsgCaller(DistributedObjectOperator Operator, OBJECT_ID SenderID, OBJECT_ID[] TargetIDList)
		{
			m_Operator = Operator;
			m_SenderID = SenderID;
		...
		public int CreateShip(SHIP_TYPE ShipType ,string ShipName )
		{
		    ...
		    uint MsgID = DistributedObject.MakeMsgID((uint)MODULE_IDS.MODULE_ID_DOSSAMPLE,(uint)DOSSAMPLE_INTERFACES.DOSSAMPLE_INTERFACE_SHIPYARD,(uint)SHIPYARD_METHODS.METHOD_CREATE_SHIP,false);		
		...
		public bool PackMsgCreateShip(CSmartStruct Packet,SHIP_TYPE ShipType ,string ShipName )
		{
			uint FailCount=0;
		
			{
				if(!Packet.AddMember((ushort)SHIPYARD_CREATE_SHIP_MEMBER_IDS.SST_CREATE_SHIP_SHIP_TYPE,(uint)ShipType)) FailCount++;
				
			}
			
			{
				if(!Packet.AddMember((ushort)SHIPYARD_CREATE_SHIP_MEMBER_IDS.SST_CREATE_SHIP_SHIP_NAME,ShipName)) FailCount++;
			}
